...
Chama obter_numero_do_usuario() para capturar entrada
Chama eh_primo() para verificar se é primo
Chama exibir_resultado() para mostrar o resultado
Trata exceções TypeError (erros de validação)
...
Testes Unitários - Criar test_rum_primo.py com pytest
Logging - Substituir print() por logging.Logger
Performance - Usar Sieve of Eratosthenes para múltiplos primos
Validação - Aceitar números grandes (BigInt)
Documentação - Gerar docs com Sphinx
Type Checking - Usar mypy para validação estática